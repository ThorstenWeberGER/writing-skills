...
**A sample overrides the style rules below.** If the sample uses em dashes freely, keep them at roughly the sample's rate, and the dash rule does not apply as a ban. **For this user the sample settles it the other way: zero em dashes across every quote, so the ban applies and `--dashes-ok` should not be passed.**
...
**Carve-out: genuine antithesis is a real device, not a tell.** A measured HBR dek reads "It isn't a failure of the technology. It's a failure of management." That is the same grammatical shape, and it is good writing, because the two halves name *two different diagnoses*, so the contrast carries information. The tell is the empty version, where both halves say the same thing and the construction only supplies emphasis ("not merely a song, it's a statement" tells you nothing new). **Test it by deleting the first half.** If you lose a real alternative that the reader might have believed, keep it. If you lose only drama, cut it.
...
**Assistant scaffolding left in the text.** *I hope this helps, Certainly!, Of course!, You're absolutely right, Would you like…, Want me to…, let me know, here is a…* Delete them; the text should stand on its own.
...
**Fake-candid openings.** *Honestly?, Look, Here's the thing, Let's be honest, Real talk*, used as standalone theatrical pauses before a routine point. (Mid-sentence "honestly" is ordinary speech; the tell is the staged opener.)
