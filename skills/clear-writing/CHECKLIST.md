...
- [ ] **Register picked before masthead, for the standfirst only.** The standfirst follows the register, not the publication: news states the next fact, a feature turns against the headline. Confirmed 6 of 6 in the FT sample. Treating this as a house difference was a recorded error. **The headline does not follow the same rule.** FT news is informational in every case, but an FT feature can be either allusive or stating, so pick the headline on whether it must survive travelling alone, not on register.
